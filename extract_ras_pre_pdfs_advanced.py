#!/usr/bin/env python3
"""
Advanced RAS PRE PDF Extractor - Recursive folder scanning
"""
...
class AdvancedRASPrePDFExtractor:
    def __init__(self):
        self.classplus_apis = {
    ...
    async def extract_pdfs_recursively(self, batch_token, folder_id=0, depth=0):
        ...
        try:
            ...
            for content in contents:
                ...
                print(f"{indent}📄 {name} (Type: {content_type})")
                ...
                elif url and any(ext in url.lower() for ext in ['.doc', '.docx', '.ppt', '.pptx', '.txt']):
                    pdfs.append({
                        'name': name,
                        'url': url,
                        'folder_depth': depth
                    })
                    print(f"{indent}  ✅ Document Found: {name}")
    ...
    async def extract_ras_pre_pdfs(self, org_code="Uievjh"):
        ...
        print("🚀 **ADVANCED RAS PRE PDF EXTRACTOR**")
        print("=" * 60)
                    ...
                    async with session.get("https://api.classplusapp.com/v2/course/preview/org/info", 
                        ...
                        if response.status == 200:
                            ...
                            if pdfs:
                                ...
                                # Save to file
                                filename = f"ras_pre_pdfs_{int(time.time())}.txt"
                                with open(filename, 'w', encoding='utf-8') as f:
                                    f.write(f"RAS PRELIMS COURSE - PDFs and Documents\n")
...
async def main():
    extractor = AdvancedRASPrePDFExtractor()
