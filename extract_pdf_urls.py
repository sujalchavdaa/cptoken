#!/usr/bin/env python3
"""
Extract actual PDF URLs from RAS PRELIMS COURSE
"""
...
class PDFURLExtractor:
    ...
    async def extract_pdfs_with_urls_recursively(self, batch_token, folder_id=0, depth=0):
        """Recursively extract PDFs with URLs from folders"""
        pdfs = []
        indent = "  " * depth
            ...
            for content in contents:
                ...
                if content_type == 3:
                    pdfs.append({
                        ...
                        'id': content.get('id'),
                        'full_content': content  # Store full content for debugging
                    })
                    print(f"{indent}📄 PDF Found: {name}")
                    if url:
                        print(f"{indent}   📎 URL: {url}")
                    else:
                        print(f"{indent}   ❌ No URL found")
                ...
                # Check if it's a folder (contentType == 1)
                    ...
                    if folder_id:
                        ...
                        folder_pdfs = await self.extract_pdfs_with_urls_recursively(batch_token, folder_id, depth + 1)
    ...
    async def get_signed_url(self, original_url):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, headers=self.classplus_headers) as response:
                    if response.status == 200:
                        data = await response.json()
                        signed_url = data.get("url") or data.get('drmUrls', {}).get('manifestUrl')
                        return signed_url
                    else:
    ...
    async def extract_ras_pre_pdfs_with_urls(self, org_code="Uievjh"):
        """Main function to extract RAS PRE PDFs with URLs"""
        print("🚀 **RAS PRE PDF URL EXTRACTOR**")
        print("=" * 60)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://api.classplusapp.com/v2/course/preview/org/info", 
                        ...
                        if response.status == 200:
                            ...
                            # Extract PDFs with URLs recursively
                            print(f"📝 Step 5: Extracting PDFs with URLs...")
                            pdfs = await self.extract_pdfs_with_urls_recursively(batch_token)
                            
                            if pdfs:
                                print(f"\n🎉 **FOUND {len(pdfs)} PDFs/DOCUMENTS WITH URLS:**")
                                print("=" * 60)
                                ...
                                for pdf in pdfs:
                                    if pdf['url']:
                                        signed_url = await self.get_signed_url(pdf['url'])
                                        pdf['signed_url'] = signed_url
                                        pdfs_with_urls.append(pdf)
                                    else:
                                        print(f"⚠️ No URL for: {pdf['name']}")
                                
                                # Save to file
                                filename = f"ras_pre_pdfs_with_urls_{int(time.time())}.txt"
                                with open(filename, 'w', encoding='utf-8') as f:
                                    f.write(f"RAS PRELIMS COURSE - PDFs with URLs\n")
                                    f.write(f"Extracted on: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                                    f.write(f"Total PDFs with URLs: {len(pdfs_with_urls)}\n")
                                    f.write(f"=" * 60 + "\n\n")
                                    ...
                                    for i, pdf in enumerate(pdfs_with_urls, 1):
                                        ...
                                        f.write(f"   Original URL: {pdf['url']}\n")
                                        if pdf['signed_url']:
                                            f.write(f"   Signed URL: {pdf['signed_url']}\n")
                                        else:
                                            f.write(f"   Signed URL: Failed to get signed URL\n")
                                ...
                                print(f"   PDFs with URLs: {len(pdfs_with_urls)}")
                                print(f"   PDFs without URLs: {len(pdfs) - len(pdfs_with_urls)}")
