#!/usr/bin/env python3
"""
Decode and analyze the tokens extracted from Classplus system (Fixed)
"""
...
class TokenAnalyzer:
    ...
    def decode_base64_fixed(self, token):
        """Decode base64 token with proper padding"""
        try:
    ...
    def analyze_hash_token(self):
        ...
        # Try to decode
        decoded = self.decode_base64_fixed(hash_token)
    ...
    def analyze_batch_token(self):
        ...
        # Try to decode
        decoded = self.decode_base64_fixed(batch_token)
        print(f"Decoded: {decoded}")
    ...
    def generate_token_info(self):
        ...
        filename = f"token_analysis_fixed_{int(time.time())}.txt"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("CLASSPLUS TOKEN ANALYSIS (FIXED)\n")
            f.write(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("=" * 60 + "\n\n")
            
            f.write("HASH TOKEN:\n")
            f.write(f"Token: {self.tokens['hash_token']}\n")
            f.write(f"Decoded: {self.decode_base64_fixed(self.tokens['hash_token'])}\n\n")
            
            f.write("BATCH TOKEN:\n")
            f.write(f"Token: {self.tokens['batch_token']}\n")
            f.write(f"Decoded: {self.decode_base64_fixed(self.tokens['batch_token'])}\n\n")
